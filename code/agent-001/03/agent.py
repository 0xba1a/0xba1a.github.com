...
ALLOWED_CATEGORIES = {
    ...
    "Web technology, front-end, back-end, react, angular, javascript, css": "Web technology, front-end, back-end, react, angular, javascript, css",
    "Other": "Other",
}

SYSTEM_PROMPT = (
    "You are an helpful assistant that explains and classifies short engineering quotes.\n"
    "Goals:\n"
    "(1) Decide whether the quote is funny or not (funny: true/false).\n"
    f"(2) Classify the quote into one of these categories: {', '.join(sorted(ALLOWED_CATEGORIES))}.\n"
    f"The classification should be one of these exact strings: {', '.join(sorted(ALLOWED_CATEGORIES))}.\n"
    "(3) And briefly explain the quote in 1 paragraph.\n"
...
def process_quote_file(path: Path, state: Dict[str, Any]) -> None:
    ...
    # Mark processed
    state.setdefault("processed", {})[file_id] = {"agent": "003", "quote": quote, "processed_at": datetime.datetime.utcnow().isoformat(), "funny": result["funny"], "explanation": result["explanation"], "category": result["category"]}
    save_state(state)
