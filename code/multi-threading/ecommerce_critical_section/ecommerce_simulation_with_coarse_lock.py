...
# ── Single coarse lock for the entire inventory ──────────────────────
...
def process_user_activity(user, activities):
    ...
    for activity in activities:
        ...
        if action == "read_spec":
            ...
            # ── WRITE inventory (value is still fresh — lock held the whole time) ──
            inventory[phone] = stock - 1
            print(f"{RED}[{timestamp()}] {user}: Purchased {phone}! (wrote stock = {stock - 1}){RESET}")
...
def main():
    ...
    print(f"\nTotal lock wait time across all threads: {total_wait_time:.2f}s")
